                    ...
                    $(i.targetElement).append(t).attr("version", e), $.ajax({
                        ...
                        success: function (e) {
                            ...
                            var t = e.approximate_member_count.toLocaleString("en") + " Members",
                                ...
                                tt = Math.floor(Math.floor(e.approximate_member_count) / 100) * 100;
                            $("#total").text(tt);
...
$(".spoiler").on("click", (event) => {
    ...
    if ($(el).hasClass("open-spoiler")) {
        $(el).removeClass("open-spoiler");
        $(el).addClass("spoiler");
    } else {
        $(el).addClass("open-spoiler");
        $(el).removeClass("spoiler");
    }
});
